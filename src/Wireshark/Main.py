...
class Tshark:
    ...
    def get_timestamp_of_first_frame_in_pcap_file(pcap_file_path) -> datetime:
        ...
        command = [
            TSHARK_BINARY, "-n", "-r", pcap_file_path, "-c", "1",
    ...
    # ) -> dict[str, Union[dict[str, list[str]], Any]]:
        ...
        if get_address_to_server_names is False and \
           get_server_name_to_addresses is False:
        ...
        pairs = set(
            p for p in [tuple(p.split(',')) for p in pairs
                        if len(p.split(',')) == 2]
        )

        if get_address_to_server_names:
            address_to_server_names = defaultdict(list)
        if get_server_name_to_addresses:
            server_name_to_addresses = defaultdict(list)

        for address, server_name in pairs:
            if get_address_to_server_names:
                address_to_server_names[address].append(server_name)
            if get_server_name_to_addresses:
                server_name_to_addresses[server_name].append(address)

        if get_address_to_server_names:
            for address in address_to_server_names:
                address_to_server_names[address].sort(
        ...
        if get_server_name_to_addresses:
            for server_name in server_name_to_addresses:
                server_name_to_addresses[server_name].sort(
    ...
    def get_endpoints_statistics_strings(
        ...
        def _parse_proto_arg(proto):
            ...
            if isinstance(proto, (str, list, set, tuple)):
                if   isinstance(proto, str):
                    protos = [p.strip() for p in proto.split(',')]
                elif isinstance(proto, (list,set,tuple)):
                    protos = proto
            else:
                raise ValueError(
                    "The 'proto' argument must be "
                    "a string or a list, set, or tuple."
                )

            unsupported_protos = [
                p for p in protos
                if p not in PROTOS_SUPPORTED_BY_ENDPOINTS_AND_CONVERSATIONS
            ]

            if unsupported_protos:
                raise ValueError(
                    "Unsupported protocols specified: "
                    f"{', '.join(unsupported_protos)}"
                )
            else:
                return protos
        ...
        def create_expression(prefix, protos, display_filter=None) -> str:
            if display_filter is None:
                return " ".join(
                    f"-z {prefix},{proto}" for proto in protos
                )
            else:
                return " ".join(
                    f"-z {prefix},{proto},'{display_filter}'"
                    for proto in protos
                )
