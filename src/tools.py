...
def cast_value(value: Any, target_type:
    ...
    if target_type == int or target_type in (AddressFamily, SocketKind):
        ...
        #     except: pass
    elif target_type == Union[IPv4Address, IPv6Address]:
        return ip_address(value)
    elif target_type == Union[IPv4Network, IPv6Network]:
...
def obj_to_stringified_dict(obj) -> dict[str, Union[int, str]]:
    ...
    def _recursive_conversion(obj):
        if isinstance(obj, (IPv4Address, IPv6Address)):
            return str(obj)
        elif isinstance(obj, (IPv4Network, IPv6Network)):
            return str(obj)
        elif isinstance(obj, dict):
            return {k: _recursive_conversion(v) for k, v in obj.items()}
        elif isinstance(obj, list):
            return list(_recursive_conversion(item) for item in obj)
        elif isinstance(obj, set):
            return sorted(_recursive_conversion(item) for item in obj)
        elif hasattr(obj, 'as_dict'):
            return _recursive_conversion(obj.as_dict())
        else:
            return obj
    return _recursive_conversion(obj.as_dict)
